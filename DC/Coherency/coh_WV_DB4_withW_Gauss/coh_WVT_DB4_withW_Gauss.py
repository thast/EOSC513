...
ax0.set_title("Coherency of $J^T$ with $S^H$, $S$ the Wavlet DB4 Transform with Source Encoding",fontsize = 22)
ax0.text(0.,1.1*coeff,"Minimum = %g"%(np.abs(mu).min()),fontsize =20)
ax0.text(0.,1.15*coeff,"Maximum = %.2f"%(np.abs(mu).max()),fontsize =20)
ax0.tick_params(labelsize = 16)

fig0.savefig('./coh_WVT_DB4_withW_Gauss.png')
