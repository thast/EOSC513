...
for i in range(coeff):
    print 'iteration: ', i
    coeffs = pywt.wavedec2(np.zeros((mesh.nCx,mesh.nCy)),'db2')
...
ax1.set_title("Coherency of $J^T$ with $S^H$, $S$ the Wavelets Transform DB2",fontsize = 22)
ax1.text(0.,55.,"Minimum = %g"%(np.abs(muwav).min()),fontsize =20)
ax1.text(0.,57.,"Maximum = %.2f"%(np.abs(muwav).max()),fontsize =20)
ax1.tick_params(labelsize = 16)

fig1.savefig('./coh_DB2_withoutW.png')
